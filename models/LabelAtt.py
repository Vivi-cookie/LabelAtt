...
class LabelAtt(nn.Module):
    #bidirectional是否使用双向的 rnn，默认是 False；batch_first：如果设置为 True，则输入数据的维度中第一个维度就 是 batch 值，默认为 False。默认情况下第一个维度是序列的长度， 第二个维度才是batch，
    def __init__(self,input_size,output_size,hidden_size,n_layers,word_vector,bidirectional=False,fine_tuned=False,datasetType=None):
        super(LabelAtt, self).__init__() #super() 函数是用于调用父类(超类)的一个方法
    ...
    def forward(self, input, seq_lengths, labels):
        ...
        input_t = input.t()#转置
        self.batch_size = input_t.size(1)
        ...
        # word embedding
        word_embedded = self.word_embedding(input_t)
        #word_embedded = self.word_embedding(input).permute(1, 0, 2) 
        ...
        #saveTextRepresentations(self.datasetType,'LabelAtt',att_output.detach().cpu().numpy())
    ...
    def label_attention(self, output, label_embedding):
        ...
        #save weights
        #save_attention_weights(self.datasetType,'LabelAtt',weights.squeeze(2).detach().cpu().numpy())
